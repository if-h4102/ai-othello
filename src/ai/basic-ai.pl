...
possibleMoves(Board, Player, PossibleMoves) :- findall([X|Y], 'end-of-game':canBePlayed(Board, X, Y, Player), PossibleMoves).
% This function returns one possible move
%useless -> canBePlayed(Board, Player, Move) :- nth0(0, Move, X), nth0(1, Move, Y), 'end-of-game':canBePlayed(Board, X, Y, Player).
...
foundBestMove(_,[],_,_,_,_) :- writeln('end'), !.
